...
Questa prossima parte se non serve non leggerla potrebbe non servire a un cazzo
...
Leggere foglio google
Esistono molti ambienti di sviluppo che permettono la risoluzione di problemi di ottimizzazione ma quelli su cui mi voglio soffermare sono il sistema automatico di CPLEX Optimization Studio e quello manuale creabile sempre con le librerie di Cplex ma implementabile con python su google colab.
La scelta di google colab come ambiente python è una scelta fatta sulla base di avere un ambiente con già le librerie installate o facilmente importabili, ambiente online quindi con sempre le cose salvate in ogni dispositivo in cui si effettua l'accesso e la modalità a celle non dovendo rifare ogni volta tutto il codice.
Come ambiente automatico intendo l'ambiente di CPLEX cioè dove non bisogna configurare i calcoli ma li fa già lui l'unica cosa da inserire sono i problemi di ottimizzazione e i dati di quei problemi poi al calcolo ci pensa lui.
In python ragioniamo in un ambiente misto nel senso che possiamo importare la libreria di cplex e definire in modo analogo i problemi e fornigli i dati ma anche configuarare da 0 un metodo di calcolo come il rilassamento lagrangiano e implementarlo.
Per l'avvio CPlex usa una parte di modello, una parte di dati e una parte di configurazione di avvio, mentre per python bisogna definire le variabili con gli elementi e definire come utilizzarle.
Inoltre python permette di stampare a immagine con la libreria ... i grafi dando un'idea più chiara del tipo di casistica che si sta affrontando
\chapter{nella parte del problema spiegare che ci sono costi e capacità}
\chapter{problem descriptio}

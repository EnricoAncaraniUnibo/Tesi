...
for soluzioneTemp in soluzioniTemp:
	...
	La terza formulazione viene implementata apportando modifiche alle funzioni utilizzate dalla seconda formulazione, in particolar modo vengono aggiunti i codici di gestione della nuova variabili decisionale $y$ secondo le modalità e condizioni proposte dalla formulazione forte definita (3.2.3).\smallskip \\
	Il metodo di calcolo della $b$ viene definito per rispecchiare la definizione fornita dalla teoria (3.15).
	{\footnotesize \begin{lstlisting}[language=Python]
...
for k in K:
	for t in NodiTarget[k]:
		...
		b[k][t][t] = -y[(k, t)]
	\end{lstlisting}}
	Si deve costruire una nuova funzione per il calcolo della condizione di accensione o spegnimento della $y$ per un determinato target e informazione:
	{\footnotesize \begin{lstlisting}[language=Python]
for k in K:
for t in NodiTarget[k]:

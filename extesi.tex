...
\begin{document}
	...
	Il problema che si vuole approfondire nello specifico è quello di una rete IoT in cui si vogliano scambiare informazioni tra dei generatori di questi dati e dei ricevitori o richiedenti.
	La rete è caratterizzata da una struttura a modello Publish and Subscribe che come definita in precedenza presenta al suo interno dei nodi di comunicazioni definiti Broker.
	L'obiettivo della tesi è andare a costruire basandomi sulla tesi di un mio collega un algoritmo efficiente per la creazione del percorso ottimale.
	Nel calcolo per la strada ottima bisogna prendere in considerazione vari fattori:
	\begin{itemize}
		\item Capacità del canale (Quante informazioni possono passare insieme)
		\item Costo del canale (Quanto è dispendioso in termini o economici o computazionali utilizzare un determinato percorso)
		\item Peso di una informazioni (Quanta banda occupa la singola informazione all'interno del canale)
	\end{itemize}
	\section{Simboli Globali}
	Per poter definire meglio il nostro problema abbiamo bisogno di incorporare quelle che sono le variabili reali in un contesto informatico/matematico:
	$K$, è l'insieme delle informazioni da inviare o ricevere.
	$N$, è il numero dei nodi totali della rete.
	$Publisher$, è l'insieme contenente i nodi che secondo il modello Publish and Subscribe fanno parte dei generatori dei flussi di informazioni.
	$Broker$, è l'insieme contenente i nodi di mezzo che non devono ne generare ne consumare le informazioni ma solo ritrasmettere.
	$Subscriber$, è l'insieme contenente i nodi che al contrario dei Publisher devono ricevere le informazioni e utilizzarle.
	$A$, è l'insieme contenente gli archi della rete quindi i vari canali di comunicazione.
	$NodoOrigine$, è l'insieme contenente per ogni informazione specificata il suo nodo di origine.
	$NodiTarget$, è l'insieme contenente per ogni informazioni specificata i nodi che richiedono quella informazione.
	$b$, è una variabile che a seconda del nodo e della informazione mi indica il ruolo di quel nodo: se è $\geq 0$ allora mi indica per quanti $NodiTarget$ sta producendo l'informazione, se è $0$ mi indica che è un nodo di trasmissione e invece se è $-1$ mi indica che è un nodo che richiede quella informazione.
	$a$, è una variabile che mi indica per ogni informazione quanta banda occupa all'interno di un canale.
	$C$, è l'insieme che associa a ogni arco quindi ad ogni collegamento un determinato costo.
	$u$, è l'insieme che associa ad ogni arco una determinata capacità che può trasportare di informazioni insieme.
	$Lt$, è l'insieme che passato un determinato nodo i mi fornisce la lista dei nodi raggiungibili da i.
	$Lr$, è l'insieme che passato un determinato nodo i mi fornisce la lista dei nodi dai quali si può raggiungere i.
	\section{first formulation}
	\section{second formulation}
	\section{Rilassamento lagrangiano nella seconda formulazione}

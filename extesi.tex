...
\begin{document}
	...
	\end{equation}
	(2.19)$\rightarrow$(2.21): Nel passaggio tra le equazioni sono state svolte due semplici operazioni: un raccoglimento dei termini comuni e l'applicazione della proprietà associativa. \smallskip \\
	...
	\end{equation}
	$LR_{i,j}(\lambda)$ indica la soluzione ottima del sottoproblema per l'arco su cui è definito.\\
	Per la sua risoluzione si introduce una variabile $d^k_{ij}$ che quantifica il costo dell'attivazione dell'arco $i,j$ per la informazione $k$:
	\begin{equation}
	...
	Il segno di $d^k_{ij}$ è necessariamente negativo poiché il Knapsack massimizza il valore degli oggetti inseriti, mentre il guadagno delle informazioni viene rappresentato come negativo.
